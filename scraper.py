...
class ScrapeBase:
    ...
    def getData(self, download=True):
        ...
        if download:
            ...
            with open(
                f"res\{self.provider}_{self.loc}_{self.getNowStr(self.now)}_page.dill",
                "wb",
            ) as f:
                dill.dump(res, f)
...
class ScrapeWeatherOnline(ScrapeBase):
    ...
    def parseHourlies(self):
        ...
        hourlies = [
            re.findall(REGEX, x.text) for x in self.hourlies_soup[0].find_all("script")
        ]
    ...
    def interestingInfosToDict(
        ...
        for info in hourly:
            ...
            if key in interesting:
                hdict[key] = val
        forecastdelta = (
            self.now.replace(hour=int(hdict["hour"])) - self.now
        ).seconds / 3600
...
class ScrapeYrNo(ScrapeBase):
    ...
    def prepareSoup(self):
        ...
        data = [
            x.string
            for x in scripts
            if x.string and x.string.startswith("\n    window.__LOCALE_CODE__ ")
        ][0]
        data = re.search("window.__REACT_QUERY_STATE__ = JSON.parse(.*)", data)
        if data:
            data = data.group(1)
            data = data[2:-3]  # strip string of (" at the beginning ") at the end
            data = (
                data.replace('\\"', '"')
                .replace("\\\\", "")
                .replace('"[', "[")
        ...
        self.soup = data or None

    def parseData(self):
        forecastData = [
            x for x in self.soup["queries"] if x["queryKey"][0] == "forecast"
        ]
        hourlies = forecastData[0]["state"]["data"]["shortIntervals"]
        xcoord, ycoord = [
            x["state"]["data"]["position"]
            for x in self.soup["queries"]
            if x["queryKey"][0] == "location"
                ][0].values()
        for hourly in hourlies:
            hour = hourly["start"]
            temp = hourly["temperature"]["value"]
            preciptiation = hourly["precipitation"]["value"]
            windkmh = hourly["wind"]["speed"] * 3.6
            pressure = hourly["pressure"]["value"]
            humidity = hourly["humidity"]["value"]
            apparentTemperature = hourly["feelsLike"]["value"]

            forecastdelta = dt.strptime(hour, "%Y-%m-%dT%H:%M:%S%z")

            return {
                "forecastdelta": forecastdelta,
                }

    # nowcast in pos 2 in soup['queries']
